...
class PeerHandler : public Event_Handler
{
    public:  
        // Initialize the client stream.
        PeerHandler(const int socket);

        // Hook method that handles communication with clients.
        virtual int handle_event(const struct epoll_event event);
...
class PeerAcceptor : public Event_Handler
{
    public:
        PeerAcceptor(const uint16_t port);
